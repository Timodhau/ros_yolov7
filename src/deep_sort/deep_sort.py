...
class DeepSort(object):
    def __init__(self, model_path, max_dist=0.3, min_confidence=0.6, nms_max_overlap=1.0, max_iou_distance=0.7, max_age=5000, n_init=5, nn_budget=100, use_cuda=True):
    ...
    def update2(self, bboxes, confidences, ori_img):
        ...
        detections = [Detection(bboxes[i], conf, features[i]) for i,conf in enumerate(confidences) if conf>self.min_confidence]
    ...
    def update(self, bbox_xywh, confidences, ori_img):
        ...
        for track in self.tracker.tracks:
            if not track.is_confirmed() or track.time_since_update > 1:
                continue
            # ERREUR ICI! LA CONVERSION EST CATASTROPHIQUE!!!!
            box = track.to_tlwh()
            x1, y1, x2, y2 = self._tlwh_to_xyxy(box)
